...
def drop_pgvector_collection(db_connection: psycopg.Connection, collection_name: str = "recipes") -> None:
    ...
    try:
        cursor = db_connection.cursor()
        # Drop the vector store collection table
        cursor.execute(f"DROP TABLE IF EXISTS langchain_pg_collection WHERE name = %s CASCADE;", (collection_name,))
        # Drop the vector data associated with the collection
        cursor.execute(f"""
